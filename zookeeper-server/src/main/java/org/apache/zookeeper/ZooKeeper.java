...
public class ZooKeeper implements AutoCloseable {
     ...
     */
    public ZooKeeper(
            //todo 服务器地址，形式为ip:port,ip:port,ip:port，中间由英文逗号分隔；也可以指定节点路径，如ip:port,ip:port,ip:port/test
            String connectString,
            //todo 会话超时时间，单位毫秒
            int sessionTimeout,
            //todo watcher事件，因为客户端建立连接时一个异步过程，需要通过注册watcher事件进行回调通知
            Watcher watcher,
            boolean canBeReadOnly,
    ...
    public ZooKeeper(
            ...
            //todo 会话ID，初次连接可以不用传
            long sessionId,
            //todo 会话密码，服务器生成会话ID的同时会分配一个会话密码，通过会话ID和密码可以重新建立连接
            byte[] sessionPasswd,
            //todo 标识当前会话是否支持只读模式
            boolean canBeReadOnly,
            //todo 服务地址选择器
            HostProvider hostProvider,
            //todo 客户端配置，可以将客户端一些配置信息如sasl、scoket实现类等放到一个文件中。一般可以不用传clientConfig
